...
public class WordUtilities {
	...
	public int binarySearchRecurse(List<String> listOfWords, String target,
		...
		if (comp < 0) return binarySearchRecurse(listOfWords, target, low, mid-1);
		
		// otherwise, target is greater than mid so check top of list recursively
		if (comp > 0) return binarySearchRecurse(listOfWords, target, mid+1, high);
	...
	public int binarySearchIterative(List<String> listOfWords, String target) {
		...
		for (int i = 0 ; i < listOfWords.size(); i++){
			mid = (low + high)/2;
			comp = target.compareTo(listOfWords.get(mid));
			if (comp == 0){
				return mid;
			}
			else if (comp < 0){
